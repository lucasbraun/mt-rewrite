...
runTPCHQueries spec setting = do
    let queries = ["SELECT L_returnflag, L_linestatus, SUM(L_quantity) AS SUM_QTY, SUM(L_extendedprice) AS SUM_BASE_PRICE, SUM(L_extendedprice*(1-L_discount)) AS SUM_DISC_PRICE, SUM(L_extendedprice*(1-L_discount)*(1+L_tax)) AS SUM_CHARGE, AVG(L_quantity) AS AVG_QTY, AVG(L_extendedprice) AS AVG_PRICE, AVG(L_discount) AS AVG_DISC, COUNT(*) AS COUNT_ORDER FROM Lineitem WHERE L_shipdate <= '1998-09-02' GROUP BY L_returnflag, L_linestatus ORDER BY L_returnflag,L_linestatus;" -- Q01
                    ...
                    ,"SELECT PS_partkey, SUM(PS_supplycost*PS_availgty) AS VALUE FROM Partsupp, Supplier, Nation WHERE PS_suppkey = S_suppkey AND S_nationkey = N_nationkey AND N_name = 'GERMANY' GROUP BY PS_partkey HAVING SUM(PS_supplycost*PS_availgty) > (SELECT SUM(PS_supplycost*PS_availgty) * 0.0001000000FROM Partsupp, Supplier, Nation WHERE PS_suppkey = S_suppkey AND S_nationkey = N_nationkey AND N_name = 'GERMANY') ORDER BY VALUE DESC;" --Q11
                    ...
                    ,"SELECT S_name, S_address FROM Supplier, Nation WHERE S_suppkey IN (SELECT PS_suppkey FROM Partsupp WHERE PS_partkey in (SELECT P_partkey FROM Part WHERE P_name like 'forest%%') AND PS_availgty > (SELECT 0.5*sum(L_quantity) FROM Lineitem WHERE L_partkey = PS_partkey AND L_suppkey = PS_suppkey AND L_shipdate >= '1994-01-01' AND L_shipdate < '1995-01-01')) AND S_nationkey = N_nationkey AND N_name = 'CANADA' ORDER BY S_name;" --Q20
