...
rewriteSelectItem spec setting p0 (Pa.SelExp ann scalExp) trefs rFun = do
    (p1,newExp) <- rewriteScalarExpr spec setting p0 scalExp trefs rFun (not (MtConversionPushUp `elem` o   = universal))
...
rewriteSelectItem spec setting p0 (Pa.SelectItem ann scalExp newName) trefs rFun = do
    (p1,h) <- rewriteScalarExpr spec setting p0 scalExp trefs rFun (not (MtConversionPushUp `elem` o   = universal))
    ...
    let triple  = getConversionFunctions spec trefs i
        ...
        rewrite (Just (to, from, (Just tName, Just attName))) =
            let (Just oldTName) = getOldTableName (Just tName) trefs
                ...
                newProv         
                | b =       let universal   = Pa.App iAnn (Pa.Name iAnn [Pa.Nmc to]) [idf, tidf]
                                rewritten
                            ...
                            in  addIdentifierToProvenance prov (to, from, (Just tName, Just attName)) idf tidf True True
                | otherwise = addIdentifierToProvenance prov (to, from (Just tName, Just attName)) idf tidf False False
            in Right $ (newProv, rewritten)
        rewrite _ _ = Right (prov, idf)
    in rewrite triple
