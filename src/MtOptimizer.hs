...
applyCDToQueryWithSplit (Pa.Select ann selDistinct (Pa.SelectList a exps) selTref selWhere
    ...
    in (Pa.Select ann selDistinct (Pa.SelectList a out) [Pa.TableAlias ann (Pa.Nmc "tmp") (Pa.SubTref ann
            (Pa.Select ann Pa.All (Pa.SelectList a (removeDuplicateSelectItems inn)) selTref selWhere
        ...
        result
            | containsString to "ToUniversal" && containsString from "FromUniversal" && isSqlAggOp appName =
                let res
                        ...
                        | otherwise =
                            let tmpString   = appName ++ "_" ++ (getIntermediateIdentifier att)
                            ...
                            in  (Pa.SelectItem iAnn (Pa.App a0 (Pa.Name a1 [Pa.Nmc from]) [
                                ...
                                [ttid])
                in  res
            | otherwise =   (Pa.SelExp iAnn newIdf,
                            [Pa.SelectItem iAnn (Pa.App ann (Pa.Name nAnn [Pa.Nmc appName]) [
                                (Pa.App a0 (Pa.Name a1 [Pa.Nmc from]) [Pa.App a2 (Pa.Name a3 [Pa.Nmc to])
                                [att, ttid], cid])]) newName],
                            [])
    in  result
...
needsSplitScalarEx (Pa.App _ (Pa.Name _ [Pa.Nmc from]) [Pa.App _ (Pa.Name _ [Pa.Nmc to]) _ ,_]) =
        containsString to "ToUniversal" && containsString from "FromUniversal"
needsSplitScalarEx (Pa.App _ (Pa.Name _ [Pa.Nmc attName]) [Pa.App a n args])
    | isCnt attName = False
    | otherwise     = needsSplitScalarEx (Pa.App a n args)
-- TODO: add more cases here if needed
